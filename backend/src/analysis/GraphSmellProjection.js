...
function mergeNodeMetadata(node, packageRef) {
  if (node.depth == null || (packageRef.depth != null && packageRef.depth < node.depth)) {
    node.depth = packageRef.depth;
  }

  if (node.dependencyType === "unknown" && packageRef.dependencyType) {
...
export function parseDirtyWatersImmediateParents(parentEvidence, affectedPackage, rootDependencyTypesByName = {}) {
  ...
  for (let index = 1; index < packageRefs.length; index += 1) {
    ...
    parents.set(toPackageNodeId(parent.name, parent.version), {
      ...parent
    });
  }
...
function getPackageEvidenceMetadata(parentEvidence, affectedPackage, rootDependencyTypesByName) {
  ...
  const depth = matches
    .map((packageRef) => packageRef.depth)
    .filter((value) => value !== null && value !== undefined)
    .sort((a, b) => a - b)[0] ?? null;

  return {
    depth,
    dependencyType: matches.find((packageRef) => packageRef.depth === depth)?.dependencyType ?? "unknown"
  };
}
